...
function isWithinTradingUpdateWindow(d) {
  const day = d.getDay();
  if (day === 0 || day === 6) {
    return false;
  }
...
async function fetchStockData(code) {
  ...
  try {
    const text = await fetchText(`https://qt.gtimg.cn/q=${marketCode}`, 5e3);
...
async function fetchStockDataBatch(codes) {
...
// src/stockConstants.ts
var PINNED_STOCKS = ["000001"];
...
// src/stockManager.ts
var STOCKS_KEY = "fafafa.stocks";
var VISIBLE_KEY = "fafafa.visible";
var REFRESH_INTERVAL = 3e3;
var StockManager = class {
  constructor(context) {
  ...
  startPolling() {
    ...
    this._interval = setInterval(() => {
      if (!isWithinTradingUpdateWindow(/* @__PURE__ */ new Date())) {
        return;
      }
...
// src/statusBarUtils.ts
var vscode2 = __toESM(require("vscode"));

// src/statusBarText.ts
var STATUS_BAR_ITEM_ID = "fafafa.status";
...
function buildStockTooltipMarkdown(codes, getData) {
  const parts = [];
...
// src/statusBarUtils.ts
function buildStockStatusTooltip(codes, getData) {
...
var StatusBarController = class {
  ...
  render() {
    ...
    const getData = (code) => this.manager.getStockData(code);
...
async function showFafafaMenu(manager, openSettings) {
...
var SettingsPanel = class _SettingsPanel {
  ...
  /** 打开或聚焦设置面板 */
  static show(extensionUri, manager) {
    if (_SettingsPanel.currentPanel) {
      _SettingsPanel.currentPanel.panel.reveal(vscode4.ViewColumn.One);
      if (!_SettingsPanel.currentPanel.isEditing) {
        _SettingsPanel.currentPanel.sendUpdate();
      }
      return;
    }
    ...
    _SettingsPanel.currentPanel = new _SettingsPanel(panel, extensionUri, manager);
  }
  /** 构建 Webview HTML */
  getHtml() {
    const webview = this.panel.webview;
    const nonce = getNonce();
    const htmlPath = vscode4.Uri.joinPath(this.extensionUri, "media", "settings.html");
    let html = fs.readFileSync(htmlPath.fsPath, "utf-8");
    const styleUri = webview.asWebviewUri(
      vscode4.Uri.joinPath(this.extensionUri, "media", "settings.css")
    );
    const iconUri = webview.asWebviewUri(
      vscode4.Uri.joinPath(this.extensionUri, "images", "icon.png")
    );
    const corePath = vscode4.Uri.joinPath(this.extensionUri, "media", "settingsCore.js");
    const bootPath = vscode4.Uri.joinPath(this.extensionUri, "media", "settings.js");
    const inlineScript = fs.readFileSync(corePath.fsPath, "utf-8") + "\n" + fs.readFileSync(bootPath.fsPath, "utf-8");
    html = html.replace(/{{cspSource}}/g, webview.cspSource).replace(/{{nonce}}/g, nonce).replace("{{styleUri}}", styleUri.toString()).replace("{{iconUri}}", iconUri.toString()).replace("{{inlineScript}}", inlineScript);
    return html;
  }
  /** 向 Webview 发送最新股票列表 */
  sendUpdate(addSuccess = false) {
...
function activate(context) {
  ...
  context.subscriptions.push(
    stockManager,
    statusBar,
    vscode5.commands.registerCommand("fafafa.openSettings", openSettings),
    vscode5.commands.registerCommand("fafafa.showMenu", () => {
      showFafafaMenu(stockManager, openSettings);
    }),
    vscode5.commands.registerCommand("fafafa.toggleVisibility", () => {
